...
def _resolve_bot_token() -> str:
    ...
    if not token:
        raise RuntimeError(
            "Telegram bot token is missing. Set TOKEN or BOT_TOKEN in the server environment."
        )
    return token
...
def _start_keep_alive_server():
    ...
    thread.start()
    logger.info("Keep-alive server started | port=%s", port)
...
def build_application() -> Application:
    ...
    application.add_handler(
        CallbackQueryHandler(
            ...
            pattern=r"^(menu:|profile:|help:|leaderboard:|support:)",
        )
    )
    application.add_handler(
        CallbackQueryHandler(
            subscribe_premium_handler,
            pattern=r"^premium:",
        )
    )
    application.add_handler(
